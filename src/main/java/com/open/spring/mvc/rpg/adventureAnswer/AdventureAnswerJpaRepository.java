...
public interface AdventureAnswerJpaRepository extends JpaRepository<AdventureAnswer, Long> {
    ...
    @Query("SELECT NEW com.open.spring.mvc.rpg.adventureAnswer.AdventureLeaderboardDto(p.id, SUM(a.chatScore)) FROM AdventureAnswer a JOIN Person p ON a.id = p.id GROUP BY p.id ORDER BY SUM(a.chatScore) DESC")
